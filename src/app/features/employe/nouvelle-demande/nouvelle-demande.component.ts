...
export class NouvelleDemandeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // ── 1. Grab current user identity ──────────────────────────────────────
    const user = this.authService.getCurrentUser();
    if (user) {
      this.currentUserEmail = user.email  ?? '';
      this.currentUserId    = user.id     ?? 0;
    }
    console.log('👤 Current user — email:', this.currentUserEmail, 'id:', this.currentUserId);

    // ── 2. Load this user's demands for duplicate detection ────────────────
    this.loadExistingDemandes();

    // ── 3. React to form changes ───────────────────────────────────────────
    this.requestForm.get('type')?.valueChanges.subscribe(type => {
      if (type === 'OCCASIONAL' || type === 'REGULAR') {
        const dateDebut = this.requestForm.get('dateDebut')?.value;
        if (dateDebut) this.requestForm.get('dateFin')?.setValue(dateDebut, { emitEvent: false });
        this.requestForm.get('dateFin')?.disable();
      } else {
        this.requestForm.get('dateFin')?.enable();
      }
      // Build calendar when switching to REGULAR
      if (type === 'REGULAR') {
        const d = this.requestForm.getRawValue().dateDebut;
        if (d) this.buildCalendar(d);
      }
      this.checkDuplicate();
    ...
    this.requestForm.get('dateDebut')?.valueChanges.subscribe(dateDebut => {
      ...
      if (type === 'OCCASIONAL' || type === 'REGULAR') {
        this.requestForm.get('dateFin')?.setValue(dateDebut, { emitEvent: false });
        this.requestForm.get('dateFin')?.disable();
      }
      // Rebuild calendar whenever dateDebut changes and type is REGULAR
      if (type === 'REGULAR' && dateDebut) this.buildCalendar(dateDebut);
      this.checkDuplicate();
      this.cdr.detectChanges();
    });
    ...
    // Default: OCCASIONAL → dateFin disabled
    this.requestForm.get('dateFin')?.disable();
    this.cdr.detectChanges();
  }
  ...
  checkDuplicate(): void {
    ...
    const conflict = this.existingDemandes.find(d => {
      ...
      console.log(`  ↳ #${d.id} [${d.type}/${d.statut}] ${d.dateDebut}→${d.dateFin} | overlap=${overlaps}`);
      return overlaps;
    });
  ...
  get isOccasional(): boolean {
    const type = this.requestForm.get('type')?.value;
    return type === 'OCCASIONAL' || type === 'REGULAR';
  }
 ...
 buildCalendar(dateStr: string): void {
  ...
  onSubmit() {
    ...
    formData.append('dateDebut', this.formatDateForBackend(raw.dateDebut));
    formData.append('dateFin',   this.formatDateForBackend(raw.dateFin));
