...
export class RhAuditComponent implements OnInit {
  ...
  // Summary counts — computed from filteredLogs
  ...
  loadLogs(): void {
    ...
    // FIX: strip the trailing 'Z' so Spring's @DateTimeFormat(iso = DATE_TIME) can parse it.
    // e.g. "2024-06-01T00:00:00.000Z" → "2024-06-01T00:00:00"
    if (this.fromDate) {
      filters.from = new Date(this.fromDate).toISOString().slice(0, 19);
    }

    if (this.toDate) {
      const to = new Date(this.toDate);
      to.setHours(23, 59, 59);
      filters.to = to.toISOString().slice(0, 19);
    }
